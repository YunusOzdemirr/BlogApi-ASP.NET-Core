    ...
    public class MailManager:IMailService
    {
        ...
        public IResult SendLandingEmail(LandingEmailDto landingEmailDto)
        {
            if (landingEmailDto.Password != "rock,paper,scissors")
                return new Result(ResultStatus.Warning, $"Bir veya daha fazla validasyon hatası ile karşılaşıldı.", new List<ValidationError> { new ValidationError { PropertyName = "Password", Message = $"Lütfen geçerli bir şifre girdikten sonra tekrar deneyiniz." } });
