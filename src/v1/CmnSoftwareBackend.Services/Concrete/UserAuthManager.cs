...
namespace CmnSoftwareBackend.Services.Concrete
{
    public class UserAuthManager : ManagerBase, IUserAuthService
    {
        ...
        public async Task<IDataResult<UserWithTokenDto>> ActiveEmailByActivationCodeAsync(UserEmailActivateDto userEmailActivateDto)
        {
            var result = ValidationTool.Validate(new UserEmailActivateDtoValidator(), userEmailActivateDto);
            if (result.ResultStatus == ResultStatus.Error)
            {
                return new DataResult<UserWithTokenDto>(ResultStatus.Warning, "Bir veya birden fazla validasyon hatası ile karşılaşıldı", null, result.ValidationErrors);
            }
            ...
            if (user != null)
            {
                if (string.Equals(userEmailActivateDto.ActivationCode, user.VerificationCode))
                {
                    ...
                    return new DataResult<UserWithTokenDto>(ResultStatus.Success,
                ...
                return new DataResult<UserWithTokenDto>(ResultStatus.Warning, $"Bir veya daha fazla validasyon hatası ile karşılaşıldı.", null, new List<ValidationError> { new ValidationError {
                        PropertyName="ActivationCode",
                        Message=$"{userEmailActivateDto.ActivationCode} numaralı aktivasyon kodu doğru değildir."
                    } });
            }
            return new DataResult<UserWithTokenDto>(ResultStatus.Warning, $"Bir veya daha fazla validasyon hatası ile karşılaşıldı.", null, new List<ValidationError> { new ValidationError {
                    PropertyName="EmailAddress",
                    Message=$"{userEmailActivateDto.EmailAddress} e-posta adresine ait bir kullanıcı bulunamadı."
                } });
        }
        ...
        public async Task<IDataResult<UserDto>> ForgotPasswordAsync(string emailAddress)
        {
            if (string.IsNullOrWhiteSpace(emailAddress))
            {
                return new DataResult<UserDto>(ResultStatus.Warning, "Bir veya daha fazla validasyon hatası ile karşılaşıldı", null, new List<ValidationError>
                {
                    new ValidationError
                    {
                        PropertyName="emailAddress",
            ...
            return new DataResult<UserDto>(ResultStatus.Warning, $"Bir veya daha fazla validasyon hatası ile karşılaşıldı.", null, new List<ValidationError> { new ValidationError {
                    PropertyName="emailAddress",
                    Message="böyle bir e posta adresi bulunamadı, geçersiz e posta adresi"
                } });
        }
        ...
        public async Task<IDataResult<UserWithTokenDto>> LoginAsync(UserLoginDto userLoginDto)
        {
            var result = ValidationTool.Validate(new UserLoginDtoValidator(), userLoginDto);
            if (result.ResultStatus == ResultStatus.Error)
            {
                return new DataResult<UserWithTokenDto>(ResultStatus.Warning, Messages.General.ValidationError(), null, result.ValidationErrors);
            }
            var user = await DbContext.Users.AsNoTracking().SingleOrDefaultAsync(u => u.EmailAddress == userLoginDto.EmailAddress);
            if (user == null)
            {
                return new DataResult<UserWithTokenDto>(ResultStatus.Warning, $"Bir veya daha fazla validasyon hatası ile karşılaşıldı.", null,
                    new List<ValidationError>
                    {
                        new ValidationError
                        {
                            PropertyName="EmailAddress & Password",
                            Message="Lütfen E-Posta adresinizi veya Şifrenizi kontrol ediniz."
                        }
                    });
            }
            if (HashingHelper.VerifyPasswordHash(userLoginDto.Password, user.PasswordHash, user.PasswordSalt))
            {
                if (!user.IsActive)
                {
                    return new DataResult<UserWithTokenDto>(ResultStatus.Warning, "Bir veya daha fazla validasyon hatası ile karşılaşıldı", null, new List<ValidationError>
                    {
                        new ValidationError
                        {
                            PropertyName="IsActive",
                            Message="Giriş yapabilmek için hesabınızın aktif olması gereklidir"
                        }
                    });
                }
                if (!user.IsEmailAddressVerified)
                {
                    var userDtoForEmailVerify = Mapper.Map<UserDto>(user);
                    return new DataResult<UserWithTokenDto>(ResultStatus.Warning, "Bir veya daha fazla validasyon hatası ile karşılaşıldı", new UserWithTokenDto
                    {
                        User = userDtoForEmailVerify
                    },
                    new List<ValidationError>
                    { new ValidationError
                      {
                        PropertyName="IsEmailAddressVerified",
                        Message="Giriş yapabilmeniz için e-posta adresinizi doğrulamanız gerekmektedir"
                      }
                    });
                }
                var accessToken = CreateAccessToken(user);
                ...
                userDto.Roles = roles;
                return new DataResult<UserWithTokenDto>(ResultStatus.Success, $"Sayın {user.FirstName} {user.LastName} hoş geldiniz.", new UserWithTokenDto
                {
                    User = userDto,
                    UserToken = Mapper.Map<AccessToken>(userToken)
                });
            ...
            return new DataResult<UserWithTokenDto>(ResultStatus.Warning, $"Bir veya daha fazla validasyon hatası ile karşılaşıldı.", null, new List<ValidationError>
            {
                ...
                {
                    PropertyName="EmailAddress & Password",
                    Message="Lütfen E-Posta adresinizi ve şifrenizi kontrol ettikten sonra tekrar deneyiniz."
                }
            });
        }

        public async Task<IDataResult<UserWithTokenDto>> RefreshUsersToken(UserRefreshTokenDto userRefreshTokenDto)
        {
            ...
            if (oldUserToken == null)
            {
                return new DataResult<UserWithTokenDto>(ResultStatus.Warning, "Bir veya daha fazla validasyon hatası ile karşılaşıldı", null, new List<ValidationError>
                {
                    new ValidationError
                    {
                        ...
                        Message="RefreshToken eksik veya hatalıdır."
                    }
                });
            }
            if (!string.Equals(userRefreshTokenDto.RefreshToken, oldUserToken.RefreshToken))
            {
                return new DataResult<UserWithTokenDto>(ResultStatus.Warning, "Bir veya daha fazla validasyon hatası ile karşılaşıldı", null, new List<ValidationError>
                {
                    ...
                    {
                        PropertyName="RefreshToken",
                        Message="RefreshToken süresi dolmuştur."
                    }
                });
            }
            if (DateTime.Now > oldUserToken.RefreshTokenExpiration)
            {
                return new DataResult<UserWithTokenDto>(ResultStatus.Warning, "Bir veya daha fazla validasyon hatası ile karşılaşıldı", null, new List<ValidationError>
                {
                    new ValidationError
                    {
                        PropertyName="RefreshTokenExpiration",
                        Message="RefreshToken eksik veya hatalıdır"
                    }
                });
            }
        ...
        public async Task<IDataResult<UserWithTokenDto>> RegisterAsync(UserRegisterDto userRegisterDto)
        {
            var result = ValidationTool.Validate(new UserRegisterDtoValidator(), userRegisterDto);
            if (result.ResultStatus == ResultStatus.Error)
            {
                return new DataResult<UserWithTokenDto>(ResultStatus.Warning, Messages.General.ValidationError(), null, result.ValidationErrors);
            }
            if (await DbContext.Users.AnyAsync(u => u.EmailAddress == userRegisterDto.EmailAddress))
            {
                return new DataResult<UserWithTokenDto>(ResultStatus.Warning, $"Bir veya daha fazla validasyon hatası ile karşılaşıldı.", null, new List<ValidationError>
                {
                    ...
                    {
                        PropertyName="EmailAddress",
                        Message=$"{userRegisterDto.EmailAddress} e-posta adresi ile kayıtlı kullanıcı mevcut."
                    }
                });
            }
            if (await DbContext.Users.AnyAsync(u => u.UserName == userRegisterDto.UserName))
            {
                return new DataResult<UserWithTokenDto>(ResultStatus.Warning, Messages.General.ValidationError(), null, new List<ValidationError>
                {
                    new ValidationError
                    {
        ...
        public async Task<IDataResult<UserDto>> ReSendActivationCodeAsync(string emailAddress)
        {
            ...
            return new DataResult<UserDto>(ResultStatus.Warning, $"Bir veya daha fazla validasyon hatası ile karşılaşıldı.", null, new List<ValidationError> { new ValidationError {
                    PropertyName="emailAddress",
                    Message="Böyle bir mail adresi bulunmamakta"
                } });
        }
